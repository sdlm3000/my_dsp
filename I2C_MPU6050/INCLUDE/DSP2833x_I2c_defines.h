//Modified From TI File $Revision: /main/1 $
...
struct I2CMSG {
  Uint16 MsgStatus;				// Word stating what state msg is in:
  								//   I2C_MSGSTAT_INACTIVE = do not send msg
  								//   I2C_MSGSTAT_BUSY = msg start has been sent,
  								//                     awaiting stop
  								//   I2C_MSGSTAT_SEND_WITHSTOP = command to send
  								//       master trans msg complete with a stop bit
  								//   I2C_MSGSTAT_SEND_NOSTOP = command to send
  								//       master trans msg without the stop bit
  								//   I2C_MSGSTAT_RESTART = command to send a restart
  								//       as a master receiver with a stop bit
  Uint16 SlaveAddress;			// I2C address of slave msg is intended for,the address of MPU should be b1101000X %X means the direction. 
  Uint16 NumOfBytes;			// Num of valid bytes in (or to be put in MsgBuffer)
  Uint16 MPUAddr;		        // MPU ROM address of data associated with msg. Refer to RM_MPU6050.pdf
