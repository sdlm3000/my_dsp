...
//		ScibRegs.SCIFFTX.bit.TXFFST=0; //FIFO队列长度（当剩余0字节时发生中断）
//		ScibRegs.SCIFFTX.bit.TXFFINT=0;//没有产生发送FIFO中断
//		ScibRegs.SCIFFTX.bit.TXFFINTCLR=0;//没有清除FIFO中断
//		ScibRegs.SCIFFTX.bit.TXFFIL=14;
//		ScibRegs.SCIFFTX.bit.TXFFIENA=0;
		ScibRegs.SCIFFTX.all=0xC028;
		ScibRegs.SCIFFRX.all=0x0028;
		ScibRegs.SCIFFCT.all=0x00;
	//    设置波特率： SCIHBAUD、SCILBAUD
	//    BRR@150MHz = LSPCLK/SCI/8-1= 37,500,000/57,600/8-1=80 (0x50)
	//    BRR@100MHz = LSPCLK/SCI/8-1= 20,000,000/57,600/8-1=42 (0x2A)
	    #if (CPU_FRQ_150MHZ)
		      ScibRegs.SCIHBAUD    =0x0000;  // 57600 baud @LSPCLK = 37.5MHz.
		      ScibRegs.SCILBAUD    =0x0050; //BAUD
		#endif
		#if (CPU_FRQ_100MHZ)
	      ScibRegs.SCIHBAUD    =0x0000;  // 57600 baud @LSPCLK = 20MHz.
	      ScibRegs.SCILBAUD    =0x002A;
		#endif

	      ScibRegs.SCICTL1.all =0x0023;  // Relinquish SCI from Reset 释放SCI资源（重启）
	//tbd...
