...
#define UARTA_RX_BUFFER_SIZE    256

Uint8 uartbRxBuffer[UARTB_RX_BUFFER_SIZE];
ringbuffer_t uartbRxFifo;

Uint8 uartaRxBuffer[UARTA_RX_BUFFER_SIZE];
ringbuffer_t uartaRxFifo;
...
//	SciaRegs.SCICCR.bit.LOOPBKENA =1; // Enable loop back
	...
	ringbuffer_init(&uartaRxFifo, uartaRxBuffer, UARTA_RX_BUFFER_SIZE);
...
void scib_int(int data_char)
{
    ...
    {
        sci_table[0]=sci_table[0]+0x30;
        scib_xmit(sci_table[0]);
    }
    if(sci_table[1]!= 0 || sci_table[0]!= 0)
    {
        sci_table[1]=sci_table[1]+0x30;
        scib_xmit(sci_table[1]);
    }
    if(sci_table[1]!= 0 || sci_table[0]!= 0 || sci_table[2]!= 0)
    {
        sci_table[2]=sci_table[2]+0x30;
        scib_xmit(sci_table[2]);
    }
    sci_table[3]=sci_table[3]+0x30;
    scib_xmit(sci_table[3]);
}

////传输浮点数
//void scib_float(double data)
//{
//    unsigned char sci_table[4];
//    long a;
//    a=data*100;                          //保留2位小数
